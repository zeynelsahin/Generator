...
namespace Generator.Business.Concrete
{
    public class ObjectParameterService : IObjectParameterService
    {
        ...
        public List<OracleColumn> GetAll(string objectId, string profileId)
        {
            ...
            var oracleColumn = result.Select(parameter => new OracleColumn
                { DataType = parameter.DataType, Name = parameter.ParameterId }).ToList();
            return oracleColumn;
        }

        public List<string> GetAllByObjectId(string objectId, string profileId)
        {
            var result = _objectParameterDal.GetAll(p => p.ObjectId == objectId && p.ProfileId == profileId)
                .Select(p => p.ParameterId).ToList();
            return result;
        }
